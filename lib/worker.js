...
function Worker(config) {
  ...
  this.master = net.connect(this.port, this.host, (err) => {
    console.log(`Connected to master node ${config.master}`)
  ...
  this.master.on('close', () => {
    console.log(`Connection closed with master node ${config.master}`)
  })

  this.master.on('error', (err) => {
    console.error(`Could not connect to master node ${config.master}:`, err.message)
  })
